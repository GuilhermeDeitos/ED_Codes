...
public:
    ...
    void setnumPoltrona(int numPoltrona, string nomeUsuario)
    {
...
private:
    No *raiz;
    No *removeNoAux(No *no, No *ant, int numPoltrona);
...
public:
    ...
    void inserirNumPoltrona(int numPoltrona);
    void inserirAux(No *no, int numPoltrona);
    void preOrdem(No *no);
    void emOrdem(No *no);
    void posOrdem(No *no);
    void removeNo(int numPoltrona);
    No* buscaPoltrona(No *no, int numPoltrona);
    void imprimeArvore()
    {
...
void Arvore::inserirNumPoltrona(int numPoltrona)
{
    if (raiz == nullptr)
    {
        raiz = new No(numPoltrona);
    }
    else
    {
        inserirAux(raiz, numPoltrona);
    }
}

void Arvore::inserirAux(No *no, int numPoltrona)
{
    if (numPoltrona < no->getValor())
    {
        if (no->getEsq() == nullptr)
        {
            no->setEsq(new No(numPoltrona));
        }
        else
        {
            inserirAux(no->getEsq(), numPoltrona);
        }
    }
    else if (numPoltrona > no->getValor())
    {
        if (no->getDir() == nullptr)
        {
            no->setDir(new No(numPoltrona));
        }
        else
        {
            inserirAux(no->getDir(), numPoltrona);
        }
    }
    else if (numPoltrona == no->getValor())
    {
...
No* Arvore::buscaPoltrona(No *no, int valor)
{
    if (no == nullptr)
    {
        return nullptr;
    ...
    {
        return buscaPoltrona(no->getEsq(), valor);
    }
    else
    {
        return buscaPoltrona(no->getDir(), valor);
    }
}
